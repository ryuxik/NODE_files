...
//-----------------------------------------------------------------------------
{
	...
	if(camera.waitForFrame())
	{
		   ...
		   init.performPixelGrouping() > 0)
		{
			...
			log(std::cout, "Looking for calib expo range, starting with max", CALIB_MAX_EXPOSURE, "us at [", 
			...
			for(; exposure >= 10 && exposure/CALIB_EXP_DIVIDER >= 1; exposure -= exposure/CALIB_EXP_DIVIDER)
			{
				...
				if(camera.waitForFrame())
				{
					   ...
					   frame.performPixelGrouping() > 0)
					{
						...
						{
							log(std::cout, "Found preferred exposure:", exposure, "us,", gain, "dB");
						...
						{
							log(std::cout, "Found lowest no-gain exposure:", exposure, "us");
					...
					{
						log(std::cerr, "Lost spot during search! Peak is at", frame.histPeak, "brightest is", frame.histBrightest);
					}
				}
				else return false;
			}
			log(std::cout, "Finished range search, lowest exposure", exposure, "us needs", gain, "dB");
			lowestExpo = exposure;
			gainMax = gain;
...
//-----------------------------------------------------------------------------
{
	...
	if(success)
	{
		...
		for(int i = 0; i < 100; i++)
		{
			// Spiral outwards with 100 points from 0 to 0.85 FSM range
			...
			if(camera.waitForFrame())
			{
				...
				if(frame.performPixelGrouping() > 0)
				{
					...
					// log(std::cout, "Pair", i, "[", frame.area.x + spot.x, ",", frame.area.y + spot.y,
					// 	"] for FSM [", x, ",", y, "]");
				...
				{
					log(std::cerr, "Lost spot during FSM calibration!");
				}
			}
		}
		...
		log(std::cout, "Calculated sensitivity matrix [", s00, s01, ";", s10, s11, "]");
		log(std::cout, "Calculated affine transform [", a00, a01, ";", a10, a11, "] + [", t0, ";", t1, "]");
		return true;
